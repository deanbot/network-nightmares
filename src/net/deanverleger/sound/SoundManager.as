package net.deanverleger.sound
{
	...
	public class SoundManager
	{
		...
		public function fadeSound($name:String, $targVolume:Number = 0, $fadeLength:Number = 1):void
		{
			...
			var transform:SoundTransform = fadeChannel.soundTransform;
			
			TweenLite.to(transform , $fadeLength, { volume: $targVolume, onUpdate: updateChannel  } );
			
			function updateChannel():void
			{
				if (transform == null || fadeChannel == null || snd == null)
					return;
				snd.volume = transform.volume;
				fadeChannel.soundTransform = transform;
			}
		}
		...
		public function unmuteAllSounds():void
		{
			for (var i:int = 0; i <this._sounds.length; i++)
			{
				...
				var curTransform:SoundTransform = SoundChannel(snd.channel).soundTransform;
				curTransform.volume = snd.volume;
				SoundChannel(snd.channel).soundTransform = curTransform;
			}
		}
		/**
